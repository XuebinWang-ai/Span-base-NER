...
class Span_NER_Model(nn.Module):

    def __init__(self, args):
        super(Span_NER_Model, self).__init__()
        ...
        if args.feat == 'bert':
            ...
            # bert-wwm
            self.feat_embed = BertEmbedding(model=args.bert_wwm_model,
                                            n_layers=args.n_bert_layers,
                                            n_out=args.n_feat_embed,
                                            requires_grad=True
                                            )
        ...
        # the MLP layers
        self.mlp_span_l = MLP(n_in=n_lstm_input,
                              n_out=args.n_mlp_span,
                              dropout=args.mlp_dropout)
        self.mlp_span_r = MLP(n_in=n_lstm_input,
                              n_out=args.n_mlp_span,  # 500
                              dropout=args.mlp_dropout)
        # the Biaffine layers
        self.span_ner_attn = Biaffine(n_in=args.n_mlp_span,  # 500
                                      n_out=args.ner_labels,
                                      bias_x=True,
                                      bias_y=True)
    ...
    def forward(self, feed_dict, link=None):
        ...
        if self.args.feat == 'bert':
            ...
            feat_embed = self.embed_dropout(feat_embed)[0]
            embed = feat_embed
        ...
        # embed: (B, L, D)
        # (B, L, hidden_size)
        x_f, x_b = embed.chunk(2, dim=-1)  # lstm前向输出与后向输出
        ...
        # (B, L-1, L-1, T)
        s_span = self.span_ner_attn(span_l, span_r).permute(0, 2, 3, 1)
        # print(s_span.shape)
        # print(s_span.nonzero().shape)
        # exit()
        return s_span
